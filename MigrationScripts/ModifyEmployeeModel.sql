...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[kenuser].[Employee]') AND [c].[name] = N'EmployeeStatusID');
IF @var24 IS NOT NULL EXEC(N'ALTER TABLE [kenuser].[Employee] DROP CONSTRAINT [' + @var24 + '];');
ALTER TABLE [kenuser].[Employee] DROP COLUMN [EmployeeStatusID];
GO
...
IF @var25 IS NOT NULL EXEC(N'ALTER TABLE [kenuser].[Employee] DROP CONSTRAINT [' + @var25 + '];');
ALTER TABLE [kenuser].[Employee] ALTER COLUMN [Company] varchar(150) NULL;
GO

ALTER TABLE [kenuser].[Employee] ADD [DepartmentCode] varchar(20) NOT NULL DEFAULT '';
GO

ALTER TABLE [kenuser].[Employee] ADD [EmployeeStatusCode] varchar(20) NULL;
GO

CREATE TABLE [kenuser].[EmployeeMasters] (
    [EmployeeNo] int NULL,
    [FirstName] nvarchar(max) NULL,
    [LastName] nvarchar(max) NULL,
    [Gender] nvarchar(max) NULL,
    [HireDate] datetime NULL,
    [EmploymentTypeCode] nvarchar(max) NULL,
    [EmploymentType] nvarchar(max) NULL,
    [ReportingManagerCode] int NULL,
    [ReportingManager] nvarchar(max) NULL,
    [DepartmentCode] nvarchar(max) NOT NULL,
    [DepartmentName] nvarchar(max) NOT NULL,
    [EmployeeStatusCode] nvarchar(max) NULL,
    [EmployeeStatus] nvarchar(max) NULL
);
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250825192057_ModifyEmployeeModel', N'8.0.17');
GO
