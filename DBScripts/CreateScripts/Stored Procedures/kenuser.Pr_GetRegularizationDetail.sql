...
BEGIN
	...
	FROM kenuser.RegularRequestWFs a WITH (NOLOCK)
		...
		LEFT JOIN kenuser.UserDefinedCode b WITH (NOLOCK) ON RTRIM(a.ROACode) = RTRIM(b.UDCCOde)
		LEFT JOIN kenuser.UserDefinedCode c WITH (NOLOCK) ON RTRIM(a.ActionCode) = RTRIM(c.UDCCOde)
